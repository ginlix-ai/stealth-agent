...
class RedisCacheClient:
    ...
    def __init__(
        ...
        """
        # Load config from conf.yaml or environment variables
        config = load_yaml_config("conf.yaml")
        redis_config = config.get("redis", {})

        # Fallback to environment variables if not in conf.yaml
        self.url = (
            url
            or redis_config.get("url")
            or os.getenv("REDIS_URL", "redis://localhost:6379/0")
        )
        self.max_connections = max_connections

        # Check if caching is enabled (conf.yaml or env var)
        cache_enabled_config = redis_config.get("cache_enabled", True)
        cache_enabled_env = os.getenv("REDIS_CACHE_ENABLED", "true").lower() in ["true", "1", "yes"]
        self.enabled = cache_enabled_config and cache_enabled_env
