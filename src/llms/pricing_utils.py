...
def detect_provider_for_model(model_name: str) -> Optional[str]:
    """
    Detect provider by reverse-looking up llm_config.json.

    Searches llm_config.json to find which provider a model_id belongs to.
    Uses case-insensitive matching to handle mismatches between API responses
    and configuration (e.g., "MiniMax-M2" vs "minimax-m2").

    Searches both:
    - Custom model names (keys in llm_config.json)
    ...
    Returns:
        Provider name if found in llm_config.json, None otherwise
    ...
    try:
        ...
        config_data = model_config.config  # Load llm_config.json
    except Exception as e:
        logger.debug(f"Failed to load llm_config.json for provider detection: {e}")
        return None
    ...
    for custom_name, config in config_data.items():
        # Check 1: Match against custom name (the key in llm_config.json)
    ...
    # Not found in llm_config.json
    logger.debug(f"No provider found for model '{model_name}' in llm_config.json")
    return None
...
def find_model_pricing(model_name: str, provider: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    if base_model != model_name:
        ...
        if pricing:
            logger.warning(
                ...
                f"llm_manifest.json if this model is frequently used."
            )
            return pricing
        else:
    ...
    if provider:
        logger.warning(
            ...
            f"Please add this model to llm_manifest.json under provider '{provider}'."
        )
    else:
        logger.warning(
            f"No pricing found for model: {model_name} (searched all providers). "
            f"Tried: exact ID match, alias match, base model '{base_model}'. "
            f"Please add this model to llm_manifest.json."
        )
