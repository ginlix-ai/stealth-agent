...
def _write_file_tool_generator(custom_description: str | None = None, *, long_term_memory: bool) -> BaseTool:
    ...
    def _write_file_to_state(state: FilesystemState, tool_call_id: str, file_path: str, content: str) -> Command:
        ...
        Returns:
            Command to update state with new file and ToolMessage (status='success' or status='error').
        """
        mock_filesystem = state.get("files", {})
            ...
            return Command(
                update={
                    "messages": [ToolMessage(
                        content=f"Error: Cannot write to {file_path} because it already exists. Read and then make an edit, or write to a new path.",
        ...
        # Build file_operation event metadata for SSE emission and database persistence
        from datetime import datetime, timezone

        # Extract current agent from state and normalize to short name
        current_agent = state.get('current_agent', 'unknown')
        # Normalize: "deep_research/coder" → "coder"
        if "/" in current_agent:
            current_agent = current_agent.split("/")[-1]

        file_event = {
            "agent": current_agent,  # Include agent for database persistence
        ...
        return Command(
            update={
                ...
                "pending_file_events": [file_event],  # Add metadata for SSE emission
                "file_operations_log": [file_event],  # Add to persistent audit trail (accumulates via reducer)
            }
        )
        ...
        def write_file(
            ...
            if _has_memories_prefix(file_path):
                ...
                return Command(
                    update={
                        "messages": [ToolMessage(
                            content=f"Successfully updated longterm memories file: {file_path}",
                            tool_call_id=runtime.tool_call_id,
                            status="success",
                            name="write_file"
                        )]
    ...
    if long_term_memory:
        ...
        def edit_file(
            ...
            *,
            ...
            # Extract current agent from runtime state and normalize to short name
            current_agent = runtime.state.get('current_agent', 'unknown')
            # Normalize: "deep_research/coder" → "coder"
            if "/" in current_agent:
                current_agent = current_agent.split("/")[-1]
...
class FilesystemMiddleware(AgentMiddleware):
    ...
    def _intercept_large_tool_result(self, tool_result: ToolMessage | Command, tool_name: str | None = None) -> ToolMessage | Command:
        if isinstance(tool_result, ToolMessage) and isinstance(tool_result.content, str):
            ...
            if self.tool_token_limit_before_evict and len(content) > 4 * self.tool_token_limit_before_evict:
                file_path = f"/large_tool_results/{tool_name}_{tool_result.tool_call_id}" if tool_name else f"/large_tool_results/{tool_result.tool_call_id}"
            ...
            for message in message_updates:
                if self.tool_token_limit_before_evict and isinstance(message, ToolMessage) and isinstance(message.content, str):
                    ...
                    if len(content) > 4 * self.tool_token_limit_before_evict:
                        file_path = f"/large_tool_results/{tool_name}_{message.tool_call_id}" if tool_name else f"/large_tool_results/{message.tool_call_id}"
                        file_data = _create_file_data(content)
    ...
    def wrap_tool_call(
        ...
        return self._intercept_large_tool_result(tool_result, request.tool_call["name"])
