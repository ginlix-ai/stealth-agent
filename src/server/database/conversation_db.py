    ...
    try:
        if conn:
            ...
            async with conn.cursor(row_factory=dict_row) as cur:
                if conversation_id:
                    await cur.execute("""
                        INSERT INTO conversation_history (conversation_id, user_id, title)
                        VALUES (%s, %s, %s)
                        RETURNING conversation_id, user_id, title, created_at, updated_at
                    """, (conversation_id, user_id, title))
                else:
                    await cur.execute("""
                        INSERT INTO conversation_history (user_id, title)
                        VALUES (%s, %s)
                        RETURNING conversation_id, user_id, title, created_at, updated_at
                    """, (user_id, title))
                result = await cur.fetchone()
                logger.info(f"Created conversation history: {result['conversation_id']} for user: {user_id}")
                return dict(result)
        else:
...
async def conversation_history_exists(conversation_id: str, conn=None) -> bool:
    """
    Check if a conversation history exists.

    Args:
        conversation_id: Conversation ID to check
        conn: Optional database connection to reuse (avoids acquiring new connection)
    """
    try:
        if conn:
            # Reuse provided connection
            async with conn.cursor(row_factory=dict_row) as cur:
                await cur.execute("""
                    SELECT EXISTS(SELECT 1 FROM conversation_history WHERE conversation_id = %s)
                """, (conversation_id,))
                result = await cur.fetchone()
...
async def update_conversation_title(conversation_id: str, title: str) -> bool:
    """Update conversation title (automatically set from first query on creation)."""
    try:
        async with get_db_connection() as conn:
            async with conn.cursor(row_factory=dict_row) as cur:
                await cur.execute("""
    ...
    try:
        async with get_db_connection() as conn:
            async with conn.cursor(row_factory=dict_row) as cur:
                ...
                # Get conversations with thread count
                query = f"""
                    SELECT
                        ch.conversation_id,
                        ch.user_id,
                    ...
                    LEFT JOIN conversation_thread t ON ch.conversation_id = t.conversation_id
                    WHERE ch.user_id = %s
                    GROUP BY ch.conversation_id
                    ORDER BY ch.{sort_by} {sort_order.upper()}
                    LIMIT %s OFFSET %s
                """
                ...
                return [dict(row) for row in conversations], total_count

    except Exception as e:
        logger.error(f"Error getting conversations for user: {e}")
        raise


# ==================== Thread Operations ====================

async def calculate_next_thread_index(conversation_id: str, conn=None) -> int:
    """
    Calculate the next thread_index for a conversation (0-based).
...
# Deprecated: Use calculate_next_thread_index instead
async def get_or_calculate_thread_index(conversation_id: str) -> int:
    """Deprecated: Use calculate_next_thread_index instead."""
    return await calculate_next_thread_index(conversation_id)
...
async def create_thread(
    ...
    if thread_index is None:
        thread_index = await calculate_next_thread_index(conversation_id, conn=conn)
    ...
    try:
        if conn:
            ...
            async with conn.cursor(row_factory=dict_row) as cur:
                await cur.execute("""
                    INSERT INTO conversation_thread (thread_id, conversation_id, current_status, msg_type, thread_index)
                    VALUES (%s, %s, %s, %s, %s)
                    RETURNING thread_id, conversation_id, current_status, msg_type, thread_index, created_at, updated_at
                """, (thread_id, conversation_id, current_status, msg_type, thread_index))
                result = await cur.fetchone()
                logger.info(f"[conversation_db] create_thread thread_id={thread_id} thread_index={thread_index} conversation_id={conversation_id}")
                return dict(result)
        else:
            # Acquire new connection (backward compatibility)
            async with get_db_connection() as conn:
...
async def ensure_conversation_and_thread_exist(
    conversation_id: str,
    thread_id: str,
    user_id: str,
    initial_query: str,
    ...
    """
    Ensure conversation_history and conversation_thread exist before workflow starts.

    Uses a single database connection for all operations to reduce connection churn.
    """
    async with get_db_connection() as conn:
        # Step 1: Ensure conversation_history exists
        exists = await conversation_history_exists(conversation_id, conn=conn)
        if not exists:
            title = initial_query[:200] if initial_query else "New Conversation"
...
async def get_threads_for_conversation(
    conversation_id: str,
    ...
    try:
        async with get_db_connection() as conn:
            async with conn.cursor(row_factory=dict_row) as cur:
                ...
                if limit:
                    await cur.execute("""
                        SELECT
                            thread_id, conversation_id, current_status, thread_index,
        ...
        logger.error(f"Error getting threads for conversation: {e}")
        raise
...
async def create_response(
    ...
    Args:
        ...
        token_usage: DEPRECATED - Token usage (use conversation_usage table instead)
                ...
                if idempotent:
                    ...
                    await cur.execute("""
                        INSERT INTO conversation_response (
                            response_id, thread_id, pair_index, final_output, status,
                            interrupt_reason, token_usage, agent_messages, metadata,
                        ...
                        SET final_output = EXCLUDED.final_output,
                            ...
                            timestamp = EXCLUDED.timestamp
                        RETURNING response_id, thread_id, pair_index, final_output, status,
                                  interrupt_reason, token_usage, agent_messages, metadata,
                                  state_snapshot, warnings, errors, execution_time, timestamp
                    """, (
                        response_id, thread_id, pair_index,
                        Json(final_output) if final_output else None,
            ...
            async with conn.cursor(row_factory=dict_row) as cur:
                ...
                if limit:
                    await cur.execute("""
                        SELECT
                            q.pair_index,
                            q.query_id, q.content as query_content, q.type as query_type,
                            q.feedback_action, q.metadata as query_metadata, q.timestamp as query_timestamp,
                            r.response_id, r.final_output, r.status, r.interrupt_reason,
                            r.token_usage, r.agent_messages, r.metadata as response_metadata,
    ...
    try:
        async with get_db_connection() as conn:
            async with conn.cursor(row_factory=dict_row) as cur:
                ...
                await cur.execute("""
                    SELECT thread_id, conversation_id, current_status, thread_index, created_at, updated_at
                    FROM conversation_thread
                    WHERE thread_id = %s
                """, (thread_id,))
                ...
                await cur.execute("""
                    SELECT
                        ...
                        COALESCE(SUM((r.token_usage->>'total_cost')::float), 0) as total_cost,
...
async def get_conversation_with_threads_enriched(conversation_id: str) -> Optional[Dict[str, Any]]:
    """Get conversation with all threads and enriched summaries."""
    try:
        async with get_db_connection() as conn:
            async with conn.cursor(row_factory=dict_row) as cur:
                ...
                threads, thread_count = await get_threads_for_conversation(conversation_id)

                # Enrich each thread with summary data
                enriched_threads = []
                total_pairs = 0
                total_cost = 0.0
                ...
                for thread_data in threads:
                    thread_summary = await get_thread_with_summary(thread_data['thread_id'])
                    if thread_summary:
                        enriched_threads.append(thread_summary)
                        total_pairs += thread_summary.get('pair_count', 0)
                        total_cost += thread_summary.get('total_cost', 0)
                        total_execution_time += thread_summary.get('total_execution_time', 0)

                conversation['thread_count'] = len(enriched_threads)
                conversation['threads'] = enriched_threads
                conversation['aggregated_stats'] = {
        ...
        logger.error(f"Error getting conversation with enriched threads: {e}")
        raise
...
async def get_user_stats(user_id: str) -> Dict[str, Any]:
    ...
    try:
        async with get_db_connection() as conn:
            async with conn.cursor(row_factory=dict_row) as cur:
                # Get conversation and thread counts
                await cur.execute("""
                    SELECT COUNT(*) as total_conversations
                    FROM conversation_history
                    WHERE user_id = %s
                """, (user_id,))
                conv_count = (await cur.fetchone())['total_conversations']
                ...
                await cur.execute("""
                    SELECT
                        ...
                        COUNT(*) as count
                    FROM conversation_history ch
                    JOIN conversation_thread t ON ch.conversation_id = t.conversation_id
                    WHERE ch.user_id = %s
        ...
        async with get_db_connection() as conn:
            async with conn.cursor(row_factory=dict_row) as cur:
                ...
                await cur.execute("""
                    ...
                    WHERE t.conversation_id = %s AND r.token_usage IS NOT NULL
                """, (conversation_id,))
...
async def get_conversation_messages(
    ...
    """Get all messages for a conversation across all threads (chronologically ordered)."""
    try:
        async with get_db_connection() as conn:
            async with conn.cursor(row_factory=dict_row) as cur:
                # Get conversation info
                ...
                # Exclude state_snapshot and token_usage for efficiency
                if limit:
                    await cur.execute("""
                        SELECT
                            t.thread_id,
                ...
                return conversation, [dict(row) for row in messages], total_count
...
async def ensure_filesystem(conversation_id: str) -> str:
    """
    Create filesystem if doesn't exist (lazy creation).

    Args:
        conversation_id: Conversation ID to create filesystem for

    Returns:
        filesystem_id (same as conversation_id)
    """
    async with get_db_connection() as conn:
        async with conn.cursor() as cur:
            # Use ON CONFLICT DO NOTHING for idempotent insert
            await cur.execute("""
                INSERT INTO conversation_filesystems (filesystem_id, conversation_id)
                VALUES (%s, %s)
                ON CONFLICT (conversation_id) DO NOTHING
            """, (conversation_id, conversation_id))

            logger.debug(f"Ensured filesystem exists for conversation {conversation_id}")
...
async def get_files_for_conversation(conversation_id: str) -> Dict[str, dict]:
    """
    Get all current files in conversation filesystem.
    ...
    async with get_db_connection() as conn:
        async with conn.cursor(row_factory=dict_row) as cur:
            await cur.execute("""
                ...
                JOIN conversation_filesystems fs ON f.filesystem_id = fs.filesystem_id
                WHERE fs.conversation_id = %s AND f.content IS NOT NULL
            """, (conversation_id,))
...
async def log_file_operation(
    ...
    async with get_db_connection() as conn:
        async with conn.cursor() as cur:
            if timestamp:
                await cur.execute("""
                    INSERT INTO conversation_file_operations (
    ...
    async def _query(connection):
        async with connection.cursor() as cur:
            await cur.execute("""
                ...
                FROM conversation_file_operations
                WHERE file_id = %s
            """, (file_id,))
            result = await cur.fetchone()
    ...
    async with get_db_connection() as conn:
        async with conn.cursor(row_factory=dict_row) as cur:
            await cur.execute("""
                ...
                FROM conversation_file_operations op
                JOIN conversation_files f ON op.file_id = f.file_id
...
async def get_file_snapshot_before_thread(conversation_id: str, thread_index: int) -> Dict[str, dict]:
    """
...
async def get_conversation_credits(conversation_id: str) -> Dict[str, Any]:
    """
    Get total credits for a conversation (across all threads and pairs).
    ...
    async with get_db_connection() as conn:
        async with conn.cursor(row_factory=dict_row) as cur:
            await cur.execute("""
                SELECT
                    %(conversation_id)s as conversation_id,
                    COALESCE(SUM(total_credits), 0) as total_credits,
                    COALESCE(SUM(token_credits), 0) as token_credits,
                    COALESCE(SUM(infrastructure_credits), 0) as infrastructure_credits,
