...
from src.tools.core.filesystem import _file_data_to_string
...
class ConversationPersistenceService:
    ...
    Usage:
        ...
        # ... workflow executes ...
        await service.persist_completion(final_output={...}, token_usage={...})
    ...
    def __init__(
        ...
        logger.debug(
            ...
            f"thread_id={thread_id} conversation_id={conversation_id} user_id={user_id}"
        )
    ...
    def get_instance(
        ...
        if thread_id not in _service_instances:
            _service_instances[thread_id] = cls(thread_id, conversation_id, user_id)
            logger.debug(f"[ConversationPersistence] Created new service instance for thread_id={thread_id}")

        # Update conversation_id and user_id if provided (may not be available at creation)
        instance = _service_instances[thread_id]
        if conversation_id and not instance.conversation_id:
    ...
    async def persist_interrupt(
        ...
        Args:
            ...
            final_output: Partial output (e.g., generated plan)
            agent_messages: Agent messages so far
            token_usage: Token usage up to interrupt
    ...
    async def persist_completion(
        ...
        Args:
            final_output: Final report/output
            token_usage: Token usage with cost breakdown
    ...
    async def persist_cancelled(
        ...
        Args:
            ...
            final_output: Partial output at cancellation
            agent_messages: Agent messages before cancellation
            token_usage: Token usage up to cancellation (summary)
    ...
    async def persist_filesystem_snapshot(
        ...
        if not self.conversation_id:
            logger.warning(
                ...
                f"conversation_id not set for thread_id={self.thread_id}"
            )
            return
        ...
        try:
            ...
            filesystem_id = await qr_db.ensure_filesystem(self.conversation_id)
            ...
            for file_path, file_data in files.items():
                ...
                for i, event in enumerate(matching_events):
                    operation = event.get('operation', 'write_file')

                    # For write_file: store full content in new_string
                    # For edit_file: store diffs in old_string/new_string
                    if operation == 'write_file':
                        old_str = None
                        new_str = event.get('content')  # Full file content
                    else:  # edit_file
                        old_str = event.get('old_string')  # Diff: replaced text
                        new_str = event.get('new_string')  # Diff: replacement text
