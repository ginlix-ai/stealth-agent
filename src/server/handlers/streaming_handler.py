...
class WorkflowStreamHandler:
    ...
    def __init__(
        ...
        self._last_subagent_status_time: float = 0.0
        self._subagent_status_interval: float = 2.0  # Emit status every 2 seconds
            ...
            async for source, data in multiplex_streams(graph_stream, keepalive_queue):
                ...
                if stream_mode == "custom":
                    if isinstance(event_data, dict):
                        ...
                        # Check if this is a file operation event from middleware
                        ...
                        if operation in ("write_file", "edit_file"):
                            ...
                            # Rebuild event with proper field order:
                            # agent, operation, file_path, tool_call_id, timestamp, status,
                            # line_count, content/old_string/new_string
                            ordered_event = {
                                "agent": agent_name,
                                "operation": operation,
                                ...
                                "line_count": event_data.get("line_count", 0),
                            }

                            # Add operation-specific fields
                            if operation == "write_file":
                                ordered_event["content"] = event_data.get("content", "")
                            elif operation == "edit_file":
                                ordered_event["old_string"] = event_data.get("old_string", "")
                                ordered_event["new_string"] = event_data.get("new_string", "")

                            # Add error field for failed events
                            if event_data.get("status") == "failed":
                                ordered_event["error"] = event_data.get("error", "Unknown error")

                            logger.info(
                                f"[FILE_OPERATION_CUSTOM] Emitting {operation} event "
                                f"(agent={agent_name}, status={ordered_event.get('status')}, "
                                f"file={ordered_event.get('file_path')}, lines={ordered_event.get('line_count', 0)})"
                            )
                            yield self._format_sse_event("file_operation", ordered_event)
                    continue
                ...
                if stream_mode == "updates":
                    if isinstance(event_data, dict):
                        ...
                        # Look inside each node's update for pending_file_events
                        for node_name, node_update in event_data.items():
                            if isinstance(node_update, dict) and "pending_file_events" in node_update:
                                ...
                                if file_events:  # Only emit if there are actually events
                                    ...
                                    logger.debug(
                                        f"[FILE_OPERATION] Emitting {len(file_events)} pending file events from {node_name} "
                                        f"(agent={agent_name})"
                                    )
                                    for event_payload in file_events:
                                        ...
                                        yield self._format_sse_event("file_operation", event_payload)
                    continue
                ...
                try:
                    ...
                    if per_call_records or tool_usage:
                        ...
                        credit_service = UsagePersistenceService(
                            thread_id=self.thread_id,
                            conversation_id="temp",  # Not needed for calculation
                            user_id="temp"
                        )
    ...
    def _maybe_emit_subagent_status(self) -> Optional[str]:
        """
        Emit background_task_status event from registry (single source of truth).
        ...
        Returns:
            SSE event string or None if not time to emit yet or no tasks
        """
        import time
        now = time.time()

        # Rate limit (every 2 seconds)
        if now - self._last_subagent_status_time < self._subagent_status_interval:
            return None
        ...
        if self._background_registry:
            ...
            self._last_subagent_status_time = now

            # Richer event with task metadata
            return self._format_sse_event("background_task_status", {
                "thread_id": self.thread_id,
                "active_tasks": [{
                    "id": t.display_id,
                ...
                "completed_tasks": [t.display_id for t in completed],
                "timestamp": datetime.now(timezone.utc).isoformat(),
            })

        # Fallback: Legacy tracking (for backward compatibility)
        if not (self._active_subagents or self._completed_subagents):
            return None
        ...
        return self._format_sse_event(
            "subagent_status",
            {
                ...
                "completed_subagents": list(self._completed_subagents),
                "timestamp": datetime.now(timezone.utc).isoformat(),
            },
        )
