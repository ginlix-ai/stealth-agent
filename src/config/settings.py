...
environment variables (.env) and YAML config files (conf.yaml).

Configuration loading strategy:
1. Credentials come from environment variables (.env)
2. Application settings come from conf.yaml
"""
...
DEFAULT_CONFIG_PATH = Path(__file__).parent.parent.parent / "conf.yaml"
...
def load_app_config(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    Args:
        config_path: Path to configuration file (defaults to conf.yaml in project root)
...
def get_config(key: str, default: Any = None, config_path: Optional[Path] = None) -> Any:
    """
    Get a configuration value from conf.yaml.
...
def get_debug_mode() -> bool:
    """Get debug mode flag from conf.yaml."""
...
def get_agent_recursion_limit(default: int = 50) -> int:
    """
    Get agent recursion limit from conf.yaml.
...
def get_workflow_timeout(default: int = 1600) -> int:
    """
    Get workflow timeout in seconds from conf.yaml.
...
def get_sse_keepalive_interval(default: float = 15.0) -> float:
    """
    Get SSE keepalive interval in seconds from conf.yaml.
...
def get_sse_event_log_level() -> str:
    """
    Get SSE event log level from conf.yaml.
...
def get_log_level() -> str:
    """
    Get root logger level from conf.yaml.
...
def get_log_format() -> str:
    """
    Get log format string from conf.yaml.
...
def get_module_log_levels() -> dict:
    """
    Get module-specific log levels from conf.yaml.
...
def get_allowed_origins() -> List[str]:
    """
    Get allowed CORS origins from conf.yaml.
...
def get_search_api() -> str:
    """
    Get configured search API from conf.yaml.

    Returns:
        Search API name (tavily, duckduckgo, brave_search, arxiv, bocha)
    """
    return str(get_config('search_api', 'tavily'))
...
def get_summarization_config() -> Dict[str, Any]:
    ...
    """
    return {
        "enabled": bool(get_nested_config('summarization.enabled', True)),
        "token_threshold": int(get_nested_config('summarization.token_threshold', 130000)),
        "keep_messages": int(get_nested_config('summarization.keep_messages', 20)),
    }


def is_summarization_enabled() -> bool:
    """Check if conversation summarization middleware is enabled."""
    return bool(get_nested_config('summarization.enabled', True))


def get_summarization_token_threshold(default: int = 130000) -> int:
    """Get token threshold for triggering summarization."""
    return int(get_nested_config('summarization.token_threshold', default))


def get_summarization_keep_messages(default: int = 20) -> int:
    """Get number of recent messages to preserve after summarization."""
    return int(get_nested_config('summarization.keep_messages', default))
