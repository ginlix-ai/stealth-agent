...
It reads settings from conf.yaml and configures the root logger as well as
...
from src.config.settings import get_log_level, get_log_format, get_module_log_levels
...
def expand_module_log_levels(raw_config: dict) -> dict:
    ...
    Args:
        raw_config: Raw module_log_levels dictionary from conf.yaml
...
def configure_logging(force: bool = False) -> None:
    """
    Configure logging based on environment settings from conf.yaml.

    This function should be called once at application startup (main.py or server startup).
    It configures:
    1. Root logger with level and format from conf.yaml
    ...
    Example:
        >>> configure_logging()  # Use settings from conf.yaml
    """
    ...
    # Get configuration from conf.yaml
