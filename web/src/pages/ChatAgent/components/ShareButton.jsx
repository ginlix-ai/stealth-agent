 ...
 */
export default function ShareButton({ threadId, initialIsShared = false }) {
  const { t } = useTranslation();
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [shareState, setShareState] = useState(null); // { is_shared, share_token, share_url, permissions }
  ...
  const handleToggleShare = useCallback(async (enabled) => {
    setUpdating(true);
    try {
  ...
  const handlePermissionChange = useCallback(async (key, value) => {
